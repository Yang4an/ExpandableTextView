...
public class ExpandableTextView extends LinearLayout {
    ...
    private int mPosition = BOTTOM_START;
...
//            mTvContentTemp.setMovementMethod(LinkMovementMethod.getInstance());
            ...
            mTvContent.setOnTouchListener(new LinkMovementMethodOverride());
            mTvContentTemp.setOnTouchListener(new LinkMovementMethodOverride());

            formatExpandText(mOriginText);
            formatCollapseText(mOriginText);
        } else if (mPosition == BOTTOM_START) {
            mTvContent.setOnTouchListener(new LinkMovementMethodOverride());
            mTvContentTemp.setOnTouchListener(new LinkMovementMethodOverride());
//            mTvContent.setMovementMethod(LinkMovementMethod.getInstance());
//            mTvContentTemp.setMovementMethod(LinkMovementMethod.getInstance());
...
//        LogUtil.d(TAG, "mOriginText" + mOriginText);
        ...
        Log.e(TAG, "toggleText:  mTvContentTemp.getLineHeight() * 3--->" + mTvContentTemp.getLineHeight() * 3);

        if (expandHeight <= mTvContentTemp.getLineHeight() * 3) {
            Log.e(TAG, "toggleText: 111");
            //说明无需折叠
            ...
            Log.e(TAG, "toggleText: 222");

            if (mPosition != ALIGN_RIGHT) {
                mTvExpand.setVisibility(VISIBLE);
            }
        }
        Log.e(TAG, "toggleText: performedByUser--->" + performedByUser);
        ...
        Log.e(TAG, "toggleText: 是否折叠的标记 mIsExpand--->" + mIsExpand);
        Log.e(TAG, "toggleText: mCancelAnim--->" + mCancelAnim);
        ...
        if (mIsExpand) {
            ...
            if (mCancelAnim) {
                collapseHeight = mTvContentTemp.getLineHeight() * 3;
                layoutParams.height = collapseHeight;
                mTvContentTemp.setLayoutParams(layoutParams);
                mTvContent.setHeight(0);
            } else {
